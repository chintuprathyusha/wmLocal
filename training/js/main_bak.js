$(document).ready(function () {
    ...
    $('.selectClass').show();
    dynamicUrl = originUrl[0]+":"+originUrl[1];
    
    // dynamicUrl = 'http://4bd12bd0.ngrok.io'  
    ...
    var retrainedData = {
        "Invoice Number": {
            ...
            "box_value": "TAX INVOICE NO. : M-014-18-1059",
            "keyword": "TAX INVOICE NO. :",
            "value": "M-014-18-1059",
            "validation": "NONE"
        },
        "Invoice Date": {
    ...
    if (nullCheck(file_name)) {
        ...
        var settings11 = {
            "async": true,
            "crossDomain": true,
            "url": dynamicUrl+":5002/get_ocr_data",
            "method": "POST",
            "processData": false,
    ...
    debugHeaderAreas = function debugHeaderAreas (event, id, areas) {
        ...
        if (!nullCheck(nextClicked)) {
            if (alt_title == 'header') {
                ...
                box_id = id+"-"+target;
                arrCount = $("."+class_name_field)[0].children.length;
                ...
                if (arrCount < a) {
                    ...
                    $("."+class_name_field).append('<div class="col-sm-12 recd-'+id+'-'+target+'" id="'+id+'" target="'+target+'" ty="new"> <div class="outputBody"> <div class="row" style="border-bottom: 1px solid #c9c9c9;height: 30px !important;"> <div class="col-sm-6 padding0"> <select class="mods_inputs keywordSelect keyword-'+id+'-'+target+'">'+default_op_optns+'</select> </div><div class="col-sm-6 padding0"> '+validation_select+' </div></div><div class="parent_main parent_main-'+id+'-'+target+'" style="height: 31px;"> <input type="text" class="inputLabel inputLabel-'+id+'-'+target+' hideInput" value=""> <div class="parent_input_here parent_input_here-'+id+'-'+target+'"></div><div class="drag drag-'+id+'-'+target+'" key="'+id+'" target="'+target+'"> <div class="triangle-down"></div><div class="triangle-up"></div></div></div><div class="row s99p"> <div class="col-sm-6"> <input class="mods_inputs keyword-final-'+id+'-'+target+'" placeholder="Keyword"> </div><div class="col-sm-6"> <input class="mods_inputs value-final-'+id+'-'+target+'" placeholder="Value"> </div></div><img src="images/md-swap.svg" class="swaping swap-'+id+'-'+target+'" key="'+id+'" target="'+target+'" width="15px"></div></div>');
    ...
    $("body").on("click", ".nextToFields", function () {
        ...
        if(retrain == "no" || (nullCheck(mainDataToSend.template.header_ocr) && nullCheck(mainDataToSend.template.footer_ocr) && nullCheck(mainDataToSend.template.address_ocr))){
    ...
    $("body").on("click", ".tryAbbyTable", function () {
        ...
        var settings11 = {
            "async": true,
            "crossDomain": true,
            "url": dynamicUrl+":5002/predict_with_ui_data",
            "method": "POST",
            "processData": false,
            "contentType": "application/json",
            "data": JSON.stringify(sendObj)
    ...
    function showRetrinedData(dt__){
        ...
        $.each(pagesObj, function(k , v){
            ...
            for(i = 0; i < v.length; i++){
                ...
                tr = '<div class="col-sm-12 recd-'+i+'-'+k+'" id="'+i+'" target="'+k+'" ty="new">'
                tr += '<div class="outputBody">'
                tr += '<div class="row" style="border-bottom: 1px solid #c9c9c9;height: 30px !important;">'
                ...
                tr += '<input type="text" class="inputLabel inputLabel-'+i+'-'+k+' hideInput" value=""> '
                tr += '<div class="parent_input_here parent_input_here-'+i+'-'+k+'">'

                for(j = 0; j < v[i].box_value.length; j++){
                    tr += '<span>'+v[i].box_value[j]+'</span>'
                }
