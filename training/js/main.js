$(document).ready(function () {
    ...
    // dynamicUrl = 'http://40648149.ngrok.io'
                ...
                if (arrCount < a) {
                    ...
                    for (var i = 0; i < default_Output_fields.length; i++) {
                        ...
                        if (default_Output_fields[i].indexOf('Table.') == -1) {
                            if (Object.values(already_selected).indexOf(default_Output_fields[i]) == -1) {
    ...
    $("body").on("change", ".keywordSelect", function () {
        val = $(this).val();
        id = $(this).closest(".fieldTrain")[0].attributes['id'].value

        target = $(this).closest(".fieldTrain")[0].attributes['target'].value
            ...
            $.ajax(settings11).done(function (msg) {
                ...
                if(!msg.flag) {
                    ...
                    $.each(msg, function(k, v){
                        ...
                        tst += '<label>'+k+'</label>'
                        tst += '<input type="text" value="'+v.replace(/suspicious/g, '')+'">'
                        tst += '</div>'
                        tst += '</div>'
    ...
    $("body").on("click", ".testBtn", function () {
        ...
        if(empt___ == 0){
            if(nullCheck($(".optionss").val()) || retrain == 'yes') {
                ...
                if (mand_check == 1) {
                    alert("Please crop "+notFoundFields.join(', ')+" fields")
                }
                else {
                    console.log(mainDataToSend);
                    obj = {}
                    obj.case_id = case_id;
                    obj.field_data = mainDataToSend.fields;
                    obj.width = $('#imageCountNum0').width();
                    obj.force_check = 'no';
